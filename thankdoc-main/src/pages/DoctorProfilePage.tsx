                    ...
                    <div className="flex space-x-2 mt-2">
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          // Immediately start Razorpay checkout when choosing Pay Online
                          try {
                            if (!doctor.consultation_fee || Number(doctor.consultation_fee) <= 0) {
                              toast({ title: 'Fee unavailable', description: 'Consultation fee not set for this doctor.', variant: 'destructive' });
                              return;
                            }
                            if (!patientDetails.name || !patientDetails.email || !patientDetails.contact) {
                              toast({ title: 'Missing details', description: 'Please fill patient name, email and contact first.', variant: 'destructive' });
                              return;
                            }
                            ...
                            console.error('Failed to initiate Razorpay', e);
                          }
                        }}
                        className="flex-1"
                      >
