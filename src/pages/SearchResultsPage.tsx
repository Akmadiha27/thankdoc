...
const SearchResultsPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      const { data, error } = await supabase.from('doctors').select('*').limit(500);
      if (!error && data) {
  ...
  // Locations derived dynamically with fallback
  const locations = (() => {
    const values = Array.from(new Set(allDoctors.map(d => d.hospital))).filter(Boolean);
